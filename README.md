...
To demo our new interpretation method, check out the The [demo/multiligtopic](./demo/multiligtopic) folder. In it is 
a csv with toy data, sentences generated by ChatGPT to be something that could be found in an oncology consultation documents.

Run this with the following. Ensure that you have added your OpenAI private key as per instructions below. 
```
python -m multiligtopic --load_sents --load_file ".\demo\multiligtopic\toy_impt_sents.txt"
```
The expected results are in [toy_impt_sents_expected.csv](./demo/multiligtopic/toy_impt_sents_expected.csv).
Note that the OpenAI representation will change over time as the model learns. 
